...
import org.scache.util.collection.OpenHashSet
 ...
 * [[org.scache.util.collection.SizeTracker]] is that, a
 * [[org.scache.util.collection.SizeTracker]] still uses [[SizeEstimator]] to
 * estimate the size. However, a [[KnownSizeEstimation]] can provide a better estimation without
 * using [[SizeEstimator]].
 */
private[scache] trait KnownSizeEstimation {
...
object SizeEstimator extends Logging {
  ...
  private def getIsCompressedOops: Boolean = {
    ...
    if (System.getProperty("scache.test.useCompressedOops") != null) {
      return System.getProperty("scache.test.useCompressedOops").toBoolean
    }
   ...
   * When alignSize = 2^n, alignSize - 1 = 2n - 1. The binary representation of (alignSize - 1)
