...
import org.scache.network.client.{RpcResponseCallback, TransportClientBootstrap, TransportClientFactory}
import org.scache.network.server._
import org.scache.network.transfer.{BlockFetchingListener, OneForOneBlockFetcher, RetryingBlockFetcher}
import org.scache.network.util.JavaUtils
import org.scache.serializer.JavaSerializer
 ...
 */
private[scache] class NettyBlockTransferService(
    conf: ScacheConf,
  ...
  private val transportConf = ScacheTransportConf.fromScacheConf(conf, "shuffle", numCores)
  ...
  private def createServer(bootstraps: List[TransportServerBootstrap]): TransportServer = {
    ...
    val portToTry = conf.getInt("scache.blockManager.port", 0)
