...
import org.scache.network.buffer.{ManagedBuffer, NettyManagedBuffer}
...
object DataReadMethod extends Enumeration with Serializable {
  ...
  val Memory, Disk, Network = Value
}


/* Class for returning a fetched block and associated metrics. */
private[scache] class BlockResult(
 ...
 */
private[scache] class BlockManager(
    executorId: String,
    rpcEnv: RpcEnv,
  ...
  // private[scache] val externalShuffleServiceEnabled =
  //   conf.getBoolean("scache.shuffle.service.enabled", false)

  val diskBlockManager = {
    // Only perform cleanup if an external service is not serving our shuffle files.
    val deleteFilesOnStop = executorId == ScacheConf.DRIVER_IDENTIFIER
  ...
  private[scache] val memoryStore =
  ...
  //   val tmpPort = Utils.getScacheOrYarnConfig(conf, "scache.shuffle.service.port", "7337").toInt
  //   if (tmpPort == 0) {
  //     // for testing, we set "scache.shuffle.service.port" to 0 in the yarn config, so yarn finds
  //     // an open port.  But we still need to tell our scache apps the right port to use.  So
  //     // only if the yarn config has the port set to 0, we prefer the value in the scache config
  //     conf.getInt("scache.shuffle.service.port", 1024)
  //   } else {
  //     tmpPort
  //   }
  // }
  ...
  private[scache] var shuffleServerId: BlockManagerId = _

  // Client to read other executors' shuffle files. This is either an external service, or just the
  // standard BlockTransferService to directly connect to other Executors.
  // private[scache] val shuffleClient = if (externalShuffleServiceEnabled) {
  //   val transConf = ScacheTransportConf.fromScacheConf(conf, "shuffle", numUsableCores)
  //   new ExternalShuffleClient(transConf, securityManager, securityManager.isAuthenticationEnabled(),
  //     securityManager.isSaslEncryptionEnabled())
  // } else {
  //   blockTransferService
  // }
  private[scache] val shuffleClient = blockTransferService

  // Max number of failures before this block manager refreshes the block locations from the driver
  private val maxFailuresBeforeLocationRefresh =
    conf.getInt("scache.block.failures.beforeLocationRefresh", 5)
  ...
  override def getBlockData(blockId: BlockId): ManagedBuffer = {
    getLocalBytes(blockId) match {
      ...
      case None => throw new Exception(blockId.toString + " not found")
    }
  }
  ...
  private def handleLocalReadFailure(blockId: BlockId): Nothing = {
    ...
    removeBlock(blockId)
    throw new Exception(s"Block $blockId was not found even though it's read-locked")
  }
      ...
      val data = try {
        ...
        case NonFatal(e) =>
          ...
          if (totalFailureCount >= maxFetchFailures) {
            ...
            throw new Exception(s"Failed to fetch block after" +
    ...
    doPutIterator(blockId, makeIterator, level, classTag, keepReadLock = true) match {
      case None =>
        ...
        val blockResult = getLocalValues(blockId).getOrElse {
          ...
          releaseLock(blockId)
          throw new Exception(s"get() failed for block $blockId even though we held a lock")
        }
  ...
  //   val syncWrites = conf.getBoolean("scache.shuffle.sync", false)
  ...
  private def getPeers(forceFetch: Boolean): Seq[BlockManagerId] = {
    peerFetchLock.synchronized {
      val cachedPeersTtl = conf.getInt("scache.storage.cachedPeersTtl", 60 * 1000) // milliseconds
...
private[scache] object BlockManager {
  ...
  def blockIdsToHosts(
    ...
    assert(blockManager != null || blockManagerMaster != null)
