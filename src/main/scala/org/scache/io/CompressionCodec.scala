...
private[scache] object CompressionCodec {

  private val configKey = "scache.io.compression.codec"

  private[scache] def supportsConcatenationOfSerializedStreams(codec: CompressionCodec): Boolean = {
  ...
  def getCodecName(conf: ScacheConf): String = {
    conf.getString(configKey, DEFAULT_COMPRESSION_CODEC)
  }

  def createCodec(conf: ScacheConf): CompressionCodec = {
    createCodec(conf, getCodecName(conf))
  }

  def createCodec(conf: ScacheConf, codecName: String): CompressionCodec = {
    val codecClass = shortCompressionCodecNames.getOrElse(codecName.toLowerCase, codecName)
    val codec = try {
      val ctor = Utils.classForName(codecClass).getConstructor(classOf[ScacheConf])
...
/**
 * :: DeveloperApi ::
 * LZ4 implementation of [[org.scache.io.CompressionCodec]].
 * Block size can be configured by `scache.io.compression.lz4.blockSize`.
 *
 * Note: The wire protocol for this codec is not guaranteed to be compatible across versions
 *       of Spark. This is intended for use as an internal compression utility within a single Spark
 *       application.
 */
class LZ4CompressionCodec(conf: ScacheConf) extends CompressionCodec {

  override def compressedOutputStream(s: OutputStream): OutputStream = {
    val blockSize = conf.getSizeAsBytes("scache.io.compression.lz4.blockSize", "32k").toInt
...
/**
 * :: DeveloperApi ::
 * LZF implementation of [[org.scache.io.CompressionCodec]].
 *
 * Note: The wire protocol for this codec is not guaranteed to be compatible across versions
 *       of Spark. This is intended for use as an internal compression utility within a single Spark
 *       application.
 */
class LZFCompressionCodec(conf: ScacheConf) extends CompressionCodec {
...
/**
 * :: DeveloperApi ::
 * Snappy implementation of [[org.scache.io.CompressionCodec]].
 * Block size can be configured by `scache.io.compression.snappy.blockSize`.
 *
 * Note: The wire protocol for this codec is not guaranteed to be compatible across versions
 *       of Spark. This is intended for use as an internal compression utility within a single Spark
 *       application.
 */
class SnappyCompressionCodec(conf: ScacheConf) extends CompressionCodec {
  val version = SnappyCompressionCodec.version

  override def compressedOutputStream(s: OutputStream): OutputStream = {
    val blockSize = conf.getSizeAsBytes("scache.io.compression.snappy.blockSize", "32k").toInt
