...
/**
 * A SASL Server for Scache which simply keeps track of the state of a single SASL session, from the
 * initial state to the "authenticated" state. (It is not a server in the sense of accepting
 * connections on some socket.)
 */
public class ScacheSaslServer implements SaslEncryptionBackend {
  private final Logger logger = LoggerFactory.getLogger(ScacheSaslServer.class);
